...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for name, module in self.features._modules.items():
            x = module(x)
            if name=='stage1' or name=='stage2' or name=='stage3' :
               outputs.append(x)
...
class BranchLast(nn.Module):
    def __init__(self, inplanes=512, width_scale=1,bits=32):
        super(BranchLast, self).__init__()
        inplanes = int(inplanes * width_scale)
        self.hash_layer = nn.Sequential(nn.Linear(inplanes, bits),nn.Tanh())
...
class BranchThird(nn.Module):
    ...
    def __init__(self, inplanes=256, width_scale=1, bits=32):
        super(BranchThird, self).__init__()
        inplanes = int(inplanes * width_scale)
        self.conv1 = nn.Conv2d(inplanes, inplanes*2, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(inplanes*2)
        self.relu = nn.ReLU(inplace=True)
        self.fc = nn.Sequential(nn.Linear(inplanes*2, bits),nn.Tanh())
...
class BranchSecond(nn.Module):
    ...
    def __init__(self, inplanes=128, width_scale=1, bits=32):
        ...
        inplanes = inplanes * 2
        self.conv2 = nn.Conv2d(inplanes, inplanes*2, kernel_size=3, stride=2, padding=1, bias=False)
...
class BranchFirst(nn.Module):
    ...
    def __init__(self, inplanes=64, width_scale=1, bits=32):
        ...
        self.bn2 = nn.BatchNorm2d(inplanes*2)
        inplanes = inplanes * 2
        self.conv3 = nn.Conv2d(inplanes, inplanes*2, kernel_size=3, stride=2, padding=1, bias=False)
...
# class my_resnet18_wd2(nn.Module):
#     def __init__(self, hash_length):
#         super(my_resnet18_wd2, self).__init__()
#         self.rnet=resnet18_wd2(pretrained=True)
#         self.rnet.output = nn.Linear(512, hash_length)
#         self.tanh=nn.Tanh()
#
#     def forward(self, x):
#         output1,output2,output3,output4=self.rnet(x)
#
