...
class FlatResNet32(FlatResNet):
    ...
    def __init__(self, block, layers, num_classes=10):
        ...
        # self.fc_logits = nn.Linear(64 * block.expansion, 10)
...
class FlatResNet32_jump(FlatResNet):

    def __init__(self, block, layers, num_classes=10):
        super(FlatResNet32_jump, self).__init__()

        #----------------256-------------------------
        self.inplanes = 16
        self.conv1 = base.conv3x3(3, 16, stride=2)
        self.bn1 = nn.BatchNorm2d(16)
        self.relu = nn.ReLU(inplace=True)
    ...
    def seed(self, x):
        x = self.maxpool(self.relu(self.bn1(self.conv1(x))))   #256
        # x = self.relu(self.bn1(self.conv1(x)))               #32
        return x
    ...
    def forward_jump(self, x, policy):
        x = self.seed(x)
        # print(x.size())
        for segment, num_blocks in enumerate(self.layer_config):
            # print((((policy>=segment).int()).sum()).data)
            if ((policy.data >=segment).int()).sum() == 0:
                break
            # last_x =x
            last_x = self.ds[segment](x)
            for b in range(num_blocks):
                residual = self.ds[segment](x) if b==0 else x
                x = F.relu(residual + self.blocks[segment][b](x))
            # print(x.size())
            # print((policy >= segment).float().view(-1,1,1,1).size())
            x = x * ((policy >= segment).float().view(-1,1,1,1)) + last_x * ((policy < segment).float().view(-1,1,1,1))
...
class Policy32(nn.Module):
    ...
    def __init__(self, layer_config=[1,1,1], num_blocks=15):
        ...
        # self.features.fc_logits = nn.Sequential()
...
class PolicySize(nn.Module):
    ...
    def __init__(self, layer_config=[1,1,1], num_out=1):
        ...
        self.logit = nn.Linear(self.feat_dim, num_out)
        self.vnet = nn.Linear(self.feat_dim, 1)

    def load_state_dict(self, state_dict):
        # support legacy models
        state_dict = {k:v for k,v in state_dict.items() if not k.startswith('features.fc')}
        return super(PolicySize, self).load_state_dict(state_dict)
...
class PolicySize_2a(nn.Module):

    def __init__(self, layer_config=[1,1,1], num_out=2):
        super(PolicySize_2a, self).__init__()
        self.features = FlatResNet32(base.BasicBlock, layer_config, num_classes=10)
        self.feat_dim = self.features.fc.weight.data.shape[1]
        self.features.fc = nn.Sequential()
        # self.features.fc_logits = nn.Sequential()
    ...
    def load_state_dict(self, state_dict):
        # support legacy models
        state_dict = {k:v for k,v in state_dict.items() if not k.startswith('features.fc')}
        return super(PolicySize_2a, self).load_state_dict(state_dict)


    def forward(self, x):
        x = self.features.forward_full(x)
        value = self.vnet(x)
        probs = F.softmax(self.logit(x))
        return probs, value
