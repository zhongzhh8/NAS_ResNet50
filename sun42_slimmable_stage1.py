...
parser.add_argument('--cv_dir', default='checkpoints_slimmable/teachers/sun42/r50_32B/11_29/all4width_batchsize100', help='checkpoint directory (models and logs are saved here)')
...
for epoch in range(start_epoch, start_epoch+args.max_epochs+1):
    ...
    for width_mult in sorted(width_mult_list, reverse=True):  # 1x,0.75x,0.5x,0.25x依次训练，用1x的soft target蒸馏训练其他
        ...
        for batch_idx, (inputs, targets) in enumerate(online_train_loader):
            ...
            time_t = time.time()
            out1,out2,out3,out=rnet(inputs)
