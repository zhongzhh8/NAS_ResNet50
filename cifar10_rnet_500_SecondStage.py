...
parser.add_argument('--cv_dir', default='cv/checkpoint2/48bits_addt3loss_margin20', help='checkpoint directory (models and logs are saved here)')
parser.add_argument('--binary_bits', type=int, default=48, help='length of hashing binary')

parser.add_argument('--batch_size', type=int, default=256, help='batch size')
parser.add_argument('--epoch_step', type=int, default=200, help='epochs after which lr is decayed')#1e-3
...
class OnlineTripletLoss_jump(nn.Module):
    ...
    def forward(self, code1, code2, code3, code4, target):
        # losses = 0.0
        # losses += (((code1- code4).pow(2).sum(1)).mean())
        # losses += (((code2 - code4).pow(2).sum(1)).mean())
        # losses += (((code3 - code4).pow(2).sum(1)).mean())
        # return losses, -1
        ...
        # losses += F.relu(ap_distances4 - an_distances4 + 20).mean()
        ...
        an_distances3 = (code3[triplets4[:, 0]] - code3[triplets4[:, 2]]).pow(2).sum(1)  # .pow(.5)
        losses += F.relu(ap_distances3 - an_distances3 + self.margin).mean()
        #试一下加上自己与自己的triplet loss能不能提高效果 (结果是效果反而更差了)
...
# rnet_checkpoint = 'pretrained/cifar10/R18_32B/ckpt_E_450_mAP_-1.00000_-1.00000_-1.00000_0.84605.t7'
...
# b4_checkpoint = 'pretrained/cifar10/R18_32B/ckpt_E_450_mAP_-1.00000_-1.00000_-1.00000_0.84605_branchLast.t7'
b4_checkpoint = torch.load(b4_checkpoint)
BranchLast.load_state_dict(b4_checkpoint)


# b1_checkpoint = '/home/disk1/azzh/PycharmProject/resnetKDs/resnetKD0/cv/tmp/ckpt_E_200_mAP_0.69476_0.75965_0.80073_0.84605_branchFirst.t7'
# b1_checkpoint = torch.load(b1_checkpoint)
# BranchFirst.load_state_dict(b1_checkpoint)
#
# b2_checkpoint = '/home/disk1/azzh/PycharmProject/resnetKDs/resnetKD0/cv/tmp/ckpt_E_200_mAP_0.69476_0.75965_0.80073_0.84605_branchSecond.t7'
# b2_checkpoint = torch.load(b2_checkpoint)
# BranchSecond.load_state_dict(b2_checkpoint)
#
# b3_checkpoint = '/home/disk1/azzh/PycharmProject/resnetKDs/resnetKD0/cv/tmp/ckpt_E_200_mAP_0.69476_0.75965_0.80073_0.84605_branchThird.t7'
# b3_checkpoint = torch.load(b3_checkpoint)
# BranchThird.load_state_dict(b3_checkpoint)
...
#这里没有训练brandlast和resnet，这样不好吧？是不是应该一起训练？ 要么也训练branchlast，要么也训练整个resnet网络？
